...
public class NetworkClient {
        ...
        client = new OkHttpClient().newBuilder().cookieJar(new CookieJar() {
            ...
            public void saveFromResponse(HttpUrl url, List<Cookie> cookies) {
                ...
                for (int i = 0; i < cookies.size(); i++) {
                    ...
                    mContext.getSharedPreferences(UInitializrApplication.SHARED_PREFRENCES, Context.MODE_PRIVATE)
    ...
    private void populateCookies() {
        Map map = mContext.getSharedPreferences(UInitializrApplication.SHARED_PREFRENCES, Context.MODE_PRIVATE).getAll();
        for (Object o : map.entrySet()) {
            ...
            if (((String) entry.getKey()).startsWith("COOKIE_")) {
                Cookie cookie = new Cookie.Builder().name(((String) entry.getKey()).substring(7))
                        ...
                        .domain(NetStatics.DOMAIN.substring(8).replace(":8080", ""))
                        .build();
    ...
    public void deleteCookies() {
        SharedPreferences sharedPreferences = mContext.getSharedPreferences(UInitializrApplication.SHARED_PREFRENCES, Context.MODE_PRIVATE);
